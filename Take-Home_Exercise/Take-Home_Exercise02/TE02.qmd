...
Before proceeding with dimensionality reduction and clustering, it is crucial to check for multicollinearity among the selected variables. K-means relies on Euclidean distance; therefore, highly correlated variables can disproportionately weight the distance calculation, inadvertently dominating the clustering results.
...
ggplot(cor_long, aes(Variable1, Variable2, fill = Correlation)) +
  ...
  labs(
    title = "Correlation Matrix of Clustering Variables (Lower Triangular)",
    x = "", y = ""
  ) +
  theme_minimal() +
  theme(
