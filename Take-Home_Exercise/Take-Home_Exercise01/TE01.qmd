...
| patchwork | An R package used to combine separate ggplot2 objects into a unified, multi-panel layout using an intuitive mathematical syntax. |
...
### 1.2 Data import and Workflow

This section summarizes how the raw survey dataset is transformed into an analytics-ready table (`clean_df`) through filtering, missing-value handling, factor recoding, and feature engineering.
...
%% ==========================================
%% 1. Professional Minimalist Style Definitions
%% ==========================================
...
%% ==========================================
A["Raw survey dataset<br/>725 respondents × 30 variables"]:::data
  --> B["Import Excel<br/>read_excel(..., na=c('', 'NA'))"]:::data

%% --- Subgraph 1: Scope (Invisible Frame) ---
subgraph S1 [" "]
  direction LR
  B --> C["Select relevant fields<br/>C*, T*, F*, LHO*, PPO*, ARO4"]:::proc
  C --> D["Duplicate audit<br/>duplicated()"]:::proc
  C --> E["Missingness scan<br/>is.na()"]:::proc
end

%% --- Subgraph 2: Cleaning (Invisible Frame) ---
subgraph S2 [" "]
  direction LR
  C --> F["Filter core outcome<br/>Keep TEIN non-missing"]:::proc
  F --> G["Rename variables<br/>total_income = TEIN"]:::proc
  G --> H["Handle non-response<br/>Housing & Policy NAs"]:::proc
  H --> I["Recode to factors<br/>+ explicit 'Unknown'"]:::proc
end

%% --- Subgraph 3: Features (Invisible Frame) ---
subgraph S3 [" "]
  direction LR
  I --> J["Daily wage<br/>income / days"]:::feat
  I --> K["Income composition<br/>agri / service / other"]:::feat
  I --> L["Aspiration index<br/>(urban + training)/2"]:::feat
end

%% --- Subgraph 4: Analytics (Invisible Frame) ---
subgraph S4 [" "]
  direction TB
...
R --> Z["Synthesis & Conclusion<br/>Integrated narrative"]:::out
...
house_df <- clean_df %>%
  filter(race != "Unknown") %>%
  select(race, house_cottage, house_roofed, house_solid, house_buildings) %>%
  pivot_longer(-race, names_to = "house_type", values_to = "status") %>%
  filter(status == "Yes") %>%
  group_by(race, house_type) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(race) %>%
  mutate(percentage = count / sum(count)) %>%
  mutate(house_type = factor(recode(house_type,
    house_cottage   = "Cottage/Temp",
    house_roofed    = "Semi-perm",
    house_solid     = "Solid/Perm",
    house_buildings = "Multi-story"),
...
theme_26_final <- theme_minimal(base_size = 10) +
  theme(
    plot.title = element_text(face = "bold", size = 12),
    axis.title = element_text(size = 9),
    axis.text.x = element_text(size = 8.5, color = "black", lineheight = 0.8), 
    legend.position = "bottom",
...
p_left <- ggplot(house_df, aes(x = race, y = percentage, fill = house_type)) +
  ...
  labs(title = "Housing Type by Race", x = "Race", y = "Share", fill = "Type") +
  theme_26_final

p_right <- ggplot(df_26_final, aes(x = house_rank, y = transition_aspiration)) +
  geom_boxplot(fill = "#547d9d", alpha = 0.9, width = 0.5, outlier.alpha = 0.1) +
  stat_summary(fun = mean, geom = "line", aes(group = 1), color = "#d8a437", linewidth = 1, linetype = "dashed") +
  stat_summary(fun = mean, geom = "point", size = 2.5, color = "#d8a437") +
  scale_x_discrete(labels = c("1. Temp\nCottage", "2. Semi\nPerm", "3. Solid\nPerm", "4. Multi\nStory")) +
  scale_y_continuous(limits = c(0.5, 5.5), breaks = 1:5) +
  labs(title = "Aspiration by Housing", x = "Housing Level", y = "Support Score") +
  coord_cartesian(clip = "off") +
  theme_26_final
...
ggplot(house_df, aes(x = race, y = percentage, fill = house_type)) +
  geom_col(position = "fill", width = 0.6, alpha = 0.9) +
  geom_text(aes(label = scales::percent(percentage, accuracy = 1)), 
            position = position_fill(vjust = 0.5), size = 3, color = "white", fontface = "bold") +
  scale_y_continuous(labels = scales::percent_format()) +
...
ggplot(df_26_final, aes(x = house_rank, y = transition_aspiration)) +
  geom_boxplot(fill = "#547d9d", alpha = 0.9, width = 0.5, outlier.alpha = 0.1) +
  stat_summary(fun = mean, geom = "line", aes(group = 1), color = "#d8a437", linewidth = 1, linetype = "dashed") +
  stat_summary(fun = mean, geom = "point", size = 2.5, color = "#d8a437") +
  scale_x_discrete(labels = c("1. Temp\nCottage", "2. Semi\nPerm", "3. Solid\nPerm", "4. Multi\nStory")) +
