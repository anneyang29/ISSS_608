...
# ---- Palette for TRAINING (not gender) ----
pal_train <- c(
  "Short course" = "#547d9d",
  "Long course"  = "#d8a437"
)

# ---- Data: drop Unknown because it's not interpretable ----
df_et <- clean_df %>%
  filter(
    !is.na(total_income), total_income >= 0,
    education != "Unknown",
  ...
  mutate(
    education = forcats::fct_relevel(
      education,
      c("Primary","Lower secondary","Upper secondary","Other")
    ),
    training_status = forcats::fct_relevel(
      training_status,
      c("Short course","Long course")
    )
  )

# ---- 99% zoom limit (keep data, only zoom display) ----
x_p99 <- as.numeric(quantile(df_et$total_income, 0.99, na.rm = TRUE))

# ---- Mean + 95% CI (within education, by training) ----
mean_df <- df_et %>%
  group_by(education, training_status) %>%
  summarise(
    n = n(),
    mean_income = mean(total_income, na.rm = TRUE),
...
p_mean_et <- ggplot(mean_df, aes(x = mean_income, y = education, color = training_status)) +
  geom_errorbarh(aes(xmin = ci_low, xmax = ci_high), height = 0.15, linewidth = 0.8) +
  geom_point(size = 3.2) +
  scale_color_manual(values = pal_train) +
  scale_x_continuous(labels = scales::comma) +
  labs(
    title = "Mean Income by Education and Training",
    subtitle = "Long-course training is associated with higher mean income",
    x = "Mean Total Income (95% CI)",
    y = "Education Level",
    color = "Training"
  ) +
  theme_minimal(base_size = 12) +
...
# ---- Distribution (boxplot) + 99% zoom ----
p_box_et <- ggplot(
  df_et,
  aes(x = total_income, y = education, fill = training_status)
) +
  geom_boxplot(
    width = 0.55,
    outlier.alpha = 0.35
  ...
  scale_x_continuous(labels = scales::comma) +
  labs(
    title = "Income Distribution by Education and Training",
    subtitle = "Median and upper-tail differences explain variation beyond the mean",
    x = "Total Income",
    y = "Education Level",
    fill = "Training"
  ) +
